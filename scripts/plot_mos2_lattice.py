...
data = pd.read_csv("data/mos2/lattice_scan.csv")
...
plt.title("MoS₂: Total Energy vs Lattice Constant")

plt.annotate(
    f"Minimum: a = {equilibrium_lattice:.2f} Å",
    (equilibrium_lattice, minimum_energy),
    xytext=(equilibrium_lattice + 0.05, minimum_energy + 0.08),
...
plt.savefig("figures/mos2_lattice_scan.png", dpi=300)
plt.show()

print(f"Equilibrium lattice constant: {equilibrium_lattice:.2f} Å")
print(f"Minimum total energy: {minimum_energy:.6f} eV")
